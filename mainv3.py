...
# Criar kernel para sharpening
sharpen_kernel = np.array([[0, -1, 0],
                           [-1, 5, -1],
                           [0, -1, 0]], dtype=np.float32)

# Aplicar sharpening em x_train
sharpened_train = []
for img in x_train:
    img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    sharpened_img = cv2.filter2D(img_bgr, -1, sharpen_kernel)
    sharpened_train.append(cv2.cvtColor(sharpened_img, cv2.COLOR_BGR2RGB))
sharpened_train = np.array(sharpened_train)

# Aplicar sharpening em x_test
sharpened_test = []
for img in x_test:
    img_bgr = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
    sharpened_img = cv2.filter2D(img_bgr, -1, sharpen_kernel)
    sharpened_test.append(cv2.cvtColor(sharpened_img, cv2.COLOR_BGR2RGB))
sharpened_test = np.array(sharpened_test)
...
for i in range(sharpened_train.shape[0]):
    img_bgr = cv2.cvtColor(sharpened_train[i], cv2.COLOR_RGB2BGR)
...
upscaled_test = []
for i in range(sharpened_test.shape[0]):
    img_bgr = cv2.cvtColor(sharpened_test[i], cv2.COLOR_RGB2BGR)
    img_upscaled = sr.upsample(img_bgr)  # Aplicar o modelo SR para upscaling
